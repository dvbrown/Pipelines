...
def generateHits(inputFile, outputFile):
    '''Turn the bam file into a pseudo bed file which then used in bedtools caoverage calculator
    with a mappable bins file generated by Parveen.'''
    comm = '''samtools view -F 4 {0} | \
    grep "XT:A:U" | \
    perl -lane 'print "$F[2]\t$F[3]"' > {2} \
    '''.format(inputFile, outputFile)
    runJob(comm, 'GENERATING HITS FILE')
    

def computeCoverage(inputFile, mappableBins, outputFile):
    '''
    '''
